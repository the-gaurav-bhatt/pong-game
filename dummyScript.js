...
var rejectButtonButton = document.getElementById("rejectButton");
...
function showGameOverScreen() {
  ...
  button.addEventListener("click", (e) => {
    e.preventDefault();
    resetFunctionality();
    socket.emit("playAgain", { response: "start", myName });
  });
}
function game() {
  body.appendChild(canvas);
...
socket.on("restartGame", (res) => {
  console.log(res);
  console.log("Refree is ", res);
  isRefree = myName === res;
  console.log("re-Starting game for :" + myName);
  // resetFunctionality();
  game();
});
socket.on("startGame", (refId) => {
  ...
  console.log("Starting game for :" + myName);
  // resetFunctionality();
  game();
});
...
socket.on("playAgain", (nameData) => {
  playAgainDiv.style.display = "block";

  acceptButton.addEventListener("click", () => {
    socket.emit("playAgain", { response: "yes", nameData });
    playAgainDiv.style.display = "none"; // Optionally, hide the container after clicking
  });

  rejectButton.addEventListener("click", () => {
    socket.emit("playAgain", { response: "no", nameData });
    playAgainDiv.style.display = "none"; // Optionally, hide the container after clicking
  });
});
